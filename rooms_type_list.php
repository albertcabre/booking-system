...
if ($request[operation]=="delete") {
	$r=mysql_query("DELETE FROM room_type WHERE room_type_id={$request[delete_room_type_id]}");
} elseif ($request[operation]=="add") {
	$r=mysql_query("INSERT INTO room_type (room_type, rate, sort) VALUES ('{$request[new_room_type]}', '{$request[new_rate]}', '{$request[new_sort]}')");
} elseif ($request[operation]=="save") {
	foreach ($request as $key => $value) {
		if (substr($key,0,9)=="room_type") {
			$room_type_id=substr($key,10);
			mysql_query("UPDATE room_type set room_type='$value' WHERE room_type_id=$room_type_id");
		}
		if (substr($key,0,4)=="rate") {
			$room_type_id=substr($key,5);
			mysql_query("UPDATE room_type set rate='$value' WHERE room_type_id=$room_type_id");
		}
		if (substr($key,0,4)=="sort") {
			$room_type_id=substr($key,5);
			mysql_query("UPDATE room_type set sort='$value' WHERE room_type_id=$room_type_id");
		}
	}
}
?>
...
<?php
$r=mysql_query("SELECT * FROM room_type ORDER BY sort");
$class="file1";
