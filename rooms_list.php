...
if ($request[operation]=="delete") {
	$r=mysqli_query("DELETE FROM rooms WHERE room_id={$request[delete_room_id]}");
} elseif ($request[operation]=="add") {
	$r=mysqli_query("INSERT INTO rooms (room, telephone) VALUES ('{$request[new_room]}', '{$request[new_telephone]}')");
} elseif ($request[operation]=="save") {
	foreach ($request as $key => $value) {
		if (substr($key,0,4)=="room") {
			$room_id=substr($key,5);
			mysqli_query("UPDATE rooms set room='$value' WHERE room_id=$room_id");
		}
		if (substr($key,0,9)=="telephone") {
			$room_id=substr($key,10);
			mysqli_query("UPDATE rooms set telephone='$value' WHERE room_id=$room_id");
		}
	}
}
?>
