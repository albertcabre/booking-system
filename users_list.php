...
if ($request[operation] == "delete") {
    $r = mysql_query("DELETE FROM user WHERE user_id=$request[delete_user_id]");
} elseif ($request[operation] == "add") {
    if (!filter_var($request[username], FILTER_VALIDATE_EMAIL)) {
        $emailErr = "Invalid email format";
    } else {
        $r = mysql_query("INSERT INTO user (username,password,sub_id) VALUES ('$request[username]','".md5($request[password])."','1')");
        $request[username] = "";
        $request[password] = "";
    }
}
...
<table align="center" border="0" cellpadding="4" cellspacing="0">
    <form name="myform" method="post" action="admin.php">
        ...
        <?php
        $r = mysql_query("SELECT * FROM user WHERE sub_id=1 ORDER BY username");
        $class = "file1";
